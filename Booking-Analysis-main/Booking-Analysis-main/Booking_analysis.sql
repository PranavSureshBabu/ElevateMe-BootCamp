...
1)
Select city, SUM(room_price * number_of_nights) as revenue from Bookings
GROUP BY city;

2)
SELECT customer_id, COUNT(*) AS total_bookings FROM Bookings
WHERE booking_date >= date('now', '-1 year')
...
4)
With city_month_revenue AS(SELECT 
	city,
...
SELECT 
  city, month, revenue
FROM city_month_revenue WHERE rnk = 1;

5)
SELECT property_id, (occupied_rooms*100)/total_rooms as occupany_rate
from properties 
GROUP BY property_id 
